...
let initFuncPath = __dirname.substring(0, __dirname.lastIndexOf("Metaprogramming-ts")) + "/Metaprogramming-ts/ts/excelToJsonTs/BaseDataManagerInitFunc.ts";
...
export function doCreateTsJsonScript(excelParsingData, projectScriptPath) {
    let dataPath = projectScriptPath +
        ...
        "/";
    let baseDataPath = projectScriptPath + "/" + JypFrameDefine.frameScriptBaseDirName + "/" + excelToTsPath + "/" + JypFrameDefine.frameReadonlyName + "/";
    ...
    if (!fs.existsSync(settingPath)) {
        let settingDataStr = canModifyTips +
            `import { ` +
