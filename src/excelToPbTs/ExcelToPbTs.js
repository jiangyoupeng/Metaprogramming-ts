...
let initFuncPath = __dirname.substring(0, __dirname.lastIndexOf("Metaprogramming-ts")) + "/Metaprogramming-ts/ts/excelToPbTs/PbBaseDataManagerInitFunc.ts";
...
export function excelToPbTs(excelParsingData, projectScriptPath, protoDirPath, overCall) {
    let dataPath = projectScriptPath +
        "/" +
        ...
        "/";
    let baseDataPath = projectScriptPath + "/" + JypFrameDefine.frameScriptBaseDirName + "/" + excelToPbPath + "/" + JypFrameDefine.frameReadonlyName + "/";
    ...
    excelParsingData.excelTableMap.forEach((tableData, key) => {
        ...
        if (!fs.existsSync(logicFilePath)) {
            let logicFileStr = canModifyTips +
                "import { " +
                className +
                " } from '../" +
    ...
    let pbExcelDataContent = canModifyTips +
