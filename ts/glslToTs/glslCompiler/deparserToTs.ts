...
function deparse_function(node: any) {
    ...
    outputPush("(")
    let returnTypes: string[] = deparse(node.children[1])
...
function deparse_functionargs(node: any) {
    ...
    let returnTypes = []
    for (var i = 0; i < len; ++i) {
        let returnType = deparse(node.children[i])
...
export function deparseToTs(
    ...
    let varingStr = "class VaringDataImpl extends VaringData {\n"
    let dataKeysStr = "    dataKeys: Map<string, any> = new Map([\n"
    let copyFuncStr = `    copy(varing: VaringDataImpl) {\n`
