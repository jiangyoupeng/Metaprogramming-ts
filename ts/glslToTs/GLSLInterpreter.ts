import * as SparkMD5 from "spark-md5/Spark-md5"
import { deparseToTs } from "./glslCompiler/deparserToTs"
import * as tokenString from "glsl-tokenizer/string"
import * as parseArray from "glsl-parser/parser"
import * as fs from "fs"
import { createAndWriteFileSync } from "../common/CommonTool"

const shaderBeginContent = `import { AttributeData, FragShaderHandle, UniformData, VaringData, VertShaderHandle, StructData } from "../ShaderDefine"
...
export class GLSLInterpreter {
    static interpreter(source: string, writePath: string) {
        let hash = SparkMD5.hash(source)
        let fileName = "Impl_" + hash

        let isWrite = false
        let files = fs.readdirSync(writePath)
        ...
        let filePath = writePath + "/" + fileName
        ...
        let token = tokenString(remainContent)
        ...
        let tsScript = deparseToTs(ast, true, "    ", uniformData, varyingData, attributeData, structDataMap, defines, isVert, hash)
        ...
        // createAndWriteFileSync(filePath, originSource)
        console.log("write " + filePath + "suc")
    }
}
