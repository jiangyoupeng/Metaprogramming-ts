...
package excelPb;
`
export function doExcelToProtoFile(excelParsingData: ExcelParsingData, protoDirPath: string, protoName?: string) {
    ...
    let pbFilePath: string = protoDirPath + "/" + protoName + ".proto"
    createAndWriteFileSync(protoDirPath)
    createAndWriteFileSync(pbFilePath, excelPbContent + totalTableContent)
}

export function excelToProtoFile(excelDirPath: string, protoDirPath: string) {
